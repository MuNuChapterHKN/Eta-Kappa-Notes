...
\begin{table}[H]
    \centering
    \begin{tabular}{l|c|c}
        Particella & Carica $Q$ & Isospin $I_3$ \\
        \hline
        $\Delta^{++}$ & $+2$ & $+3/2$ \\
        $\Delta^{+}$ & $+1$ & $+1/2$ \\
        $\Delta^{0}$ & $0$ & $-1/2$ \\
...
\begin{equation}
    \brackets{-\lap + \dfrac{m_{\pi}^2 c^2}{\hbar^2}} \phi(\vec{r}) = 4\pi g_{\pi}\delta(\vec{r})
\end{equation}
...
\end{equation}
Dove $g_{\pi}$ è una costante di accoppiamento che determina la forza dell'interazione. Il potenziale di Yukawa descrive una forza a corto raggio, che decresce esponenzialmente con la distanza $r$ come possiamo osservare:
...
\begin{equation}
  \begin{split}
    &E_A^2 = p_A^2 c^2 + M_A^2 c^4 \\[8pt]
    &E_X^2 = p_A^2 c^2 + m_X^2 c^4
  \end{split}
\end{equation}
Il modulo del quadrivettore della particella $A$ risulta dunque essere:
\begin{equation}
    p_A^{\mu} p_{A \mu} = E_A^2 - p_A^2 c^2 = M_A^2 c^4
\end{equation}
Ma allora la differenza tra l'energia iniziale e finale risulta:
\begin{equation}
    \Delta E_A =  E_A + E_X - M_A c^2 = \sqrt{p_A^2 c^2 + M_A^2 c^4} + \sqrt{p_A^2 c^2 + M_X^2 c^4} - M_A c^2
    \begin{cases}
       \approx 2pc & \text{se } p_A \to 0 \\
       \approx M_X c^2 & \text{se } p_A \to \infty
    \end{cases}
\end{equation}
Dunque l'energia risulta non conservata per ogni valore di $p_A$. Possiamo dunque applicare il principio di indeterminazione di Heisenberg per giustificare la presenza della particella $X$ per un tempo limitato:
\begin{equation}
    \Delta E_A \Delta t \approx \hbar \implies \Delta t \approx \dfrac{\hbar}{\Delta E_A}
\end{equation}
...
\begin{equation}
    r \approx c \tau = \dfrac{\hbar c}{\Delta E_A}
\end{equation}
Poichè $r$ è inversamente proporzionale a $\Delta E_A$, possiamo trovare un valore limite nel caso di $p_A \to 0$:
\begin{equation}
    r_{\text{max}} \approx \dfrac{\hbar c}{M_X c^2} = \dfrac{\hbar}{M_X c} = \dfrac{1}{\lambda_X}
\end{equation}
...
\addfigure[Schema di funzionamento di una camera a bolle]{drawio-pdf/Camera_a_bolle.drawio}{0.6}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Con $m_{\varrho} \approx \qty{770}{\mega\electronvolt}$ e $m_{\omega} \approx \qty{782}{\mega\electronvolt}$.\\
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        \hline \\[-8pt]
        Nome & Simbolo & Carica $Q$ & Massa (GeV) & Stranezza $S$\\[4pt]
        \hline \\[-8pt]
        Down & $d$ & $-1/3$ & $m_d \approx 0.3$ & $0$\\[8pt]
        Up & $u$ & $+2/3$ & $m_u \approx m_d$ & $0$\\[8pt]
        Strange & $s$ & $-1/3$ & $m_s \approx 0.5$ & $-1$\\[8pt]
        Charm & $c$ & $+2/3$ & $m_c \approx 1.5$ & $0$\\[8pt]
        Bottom & $b$ & $-1/3$ & $m_b \approx 4.5$ & $0$\\[8pt]
        Top & $t$ & $+2/3$ & $m_t \approx 180 \pm 12$ & $0$
    \end{tabular}
...
I gluoni hanno $J =1$ e massa nulla, e trasportano una combinazione di colore e anticolore. Ci sono nove tipi di gluoni ($3 \times 3$), ciascuno con una diversa combinazione di colori:
\begin{equation}
    \begin{split}
      &\text{Ottetti di gluoni:} \\[8pt]
      &r \anti{g}, \qquad r \anti{b}, \qquad g \anti{b}, \qquad g \anti{r}, \qquad b \anti{g}, \qquad b \anti{r}, \qquad \dfrac{1}{\sqrt{2}} (r \anti{r} - g \anti{g}), \qquad  \dfrac{1}{\sqrt{6}} (r \anti{r} + g \anti{g} - 2 b \anti{b}) \\[8pt]
      &\text{Singoletto di gluone:} \\[8pt]
