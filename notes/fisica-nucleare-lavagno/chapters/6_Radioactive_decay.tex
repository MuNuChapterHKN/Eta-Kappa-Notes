...
Possiamo anche definire l'attività $\mathcal{A}$ come:
\begin{definition}{Attività}
    L'\textbf{attività} $\mathcal{A}(t)$ di un campione radioattivo è definita come il numero di decadimenti per unità di tempo:
    \begin{equation}
...
\begin{equation}
    \tau = \avg{\mathcal{A}(t)} = \dfrac{\lambda \mathcal{N}_0 \int_0^{+\infty} t \efunction^{-\lambda t}}{\lambda \mathcal{N}_0 \int_0^{+\infty} \efunction^{-\lambda t}} \dd{t} = \lambda \int_0^{+\infty} t \efunction^{-\lambda t} = \frac{1}{\lambda}
\end{equation}
...
Infine definiamo il tempo di dimezzamento $T_{1/2}$ come:
\begin{definition}{Tempo di dimezzamento}
    Il \textbf{tempo di dimezzamento} $T_{1/2}$ è l'intervallo di tempo necessario affinché la quantità di nuclei si riduca alla metà del valore iniziale.
    \begin{equation}
...
\addsvg[Decadimento radioattivo e tempo di dimezzamento]{Radioactive_decay}{0.9}
...
\begin{equation}
    Q = K_\alpha + K_Y
\end{equation}
Tipicamente $K_Y \ll K_\alpha$ poiché la massa del nucleo figlio è molto maggiore di quella della particella $\alpha$. Inoltre $K_\alpha \simeq \qty{5}{\mega\electronvolt}$. Ad esempio, nel decadimento dell'uranio-238 in torio-234, l'energia di reazione è $Q = \qty{4.27}{\mega\electronvolt}$, di cui $\qty{4.25}{\mega\electronvolt}$ sono portati dalla particella $\alpha$. La reazione è la seguente:
\begin{equation}
    \atom{U}{238}{92} \to \atom{Th}{234}{90} + \alpha
\end{equation}
Caratterizzata da un tempo di dimezzamento di $T_{1/2} = 4.47 \cdot 10^9 \text{ anni}$.\\
...
\addtikz[Catena di decadimento dell'uranio-238]{
    \foreach \x / \xlabel in {0 / 81,1 / 82, 2 / 83, 3 / 84,4 / 85,5 / 86,6 / 87,7 / 88,8 / 89,9 / 90,10 / 91, 11 / 92} {
        \draw (\x,0)node[below] {\xlabel} -- (\x,16);
    }
    \foreach \x / \xlabel in {0 / Tl,1 / Pb, 2 / Bi, 3 / Po,4 / At,5 / Rn,6 / Fr,7 / Ra,8 / Ac,9 / Th,10 / Pa, 11 / U} {
        \node[below] at (\x,-0.5) {\xlabel};
    }
    \foreach \y / \ylabel in {0 / 206,2 / 210, 4 / 214,6 / 218,8 / 222,10 / 226,12 / 230,14 / 234,16 / 238} {
        \draw (0,\y)node[left,yshift=2] {\ylabel} -- (11,\y);
    }
    \node[above] at (0,16.5){$A$};
    \node[right] at (11.5,0){$Z$};
    %Alpha decays
    \draw[multiarrows,thick,red] (11,16)node[above left,xshift=2]{$\atom{U}{238}{}$} -- ++(-2,-2)node[above left,xshift=2]{$\atom{Th}{234}{}$} (11,14) -- ++(-2,-2)node[above left,xshift=2]{$\atom{Th}{230}{}$} (9,12) -- ++(-2,-2)node[above left,xshift=2]{$\atom{Ra}{226}{}$} (7,10) -- ++(-2,-2)node[above left,xshift=2]{$\atom{Rn}{222}{}$} (5,8) -- ++(-2,-2)node[above left,xshift=2]{$\atom{Po}{218}{}$} (3,6) -- ++(-2,-2)node[above left,xshift=2]{$\atom{Pb}{214}{}$} (4,6) -- ++(-2,-2) (5,6) -- ++(-2,-2) (2,4) -- ++(-2,-2) (3,4) -- ++(-2,-2) (2,2) -- ++(-2,-2) (3,2) -- ++(-2,-2);
    %Beta decays
    \draw[multiarrows,thick,teal] (3,6) -- ++(1,0) (4,6) -- ++(1,0) (1,4) -- ++(1,0) (2,4) -- ++(1,0) (0,2) -- ++(1,0) (1,2) -- ++(1,0) (2,2) -- ++(1,0) (9,14) -- ++(1,0) (10,14) -- ++(1,0);
    %Legenda
    \draw[->,thick,red] (3,14) -- ++(-2,-2) node[below right]{Decadimento $\alpha$};
    \draw[->,thick,teal] (3,14) -- ++(1,0) node[below right]{Decadimento $\beta^-$};
    \fill[white] (1.5,14.25) rectangle (4.5,15.25);
    \draw[thick] (1.5,14.25) rectangle (4.5,15.25);
    \node at (3,14.75){Legenda};


}{1}{uranium_decay_chain}
...
\addtikz[Decadimento $\alpha$ come fenomeno di tunneling quantistico]{
    ...
    \draw[dashed,thick] (0,3) node[left]{$\simeq \qty{30}{\mega\electronvolt}$} -- (1.5,3);
    \node[left] at (0,-4) {$\simeq -\qty{40}{\mega\electronvolt}$};
...
\begin{equation}
    T \simeq \efunction^{-2\gamma L} \quad \text{con} \quad \gamma = \frac{\sqrt{2m(V_0 - E)}}{\hbar}
\end{equation}
...
\begin{equation}
    \begin{split}
        & 2\gamma L = 2\dfrac{\sqrt{2 \cdot \qty{9.11e-31}{\kilo\gram} \cdot (\qty{40}{\electronvolt} - \qty{30}{\electronvolt}) \cdot \qty{1.6e-19}{\joule\per\electronvolt}}}{\qty{1.05e-34}{\joule\second}} \cdot \qty{1e-9}{\meter} \simeq 32.5 \\[8pt]
        & T \simeq \efunction^{-32.5} \simeq 8.5 \cdot 10^{-15} \rightarrow \text{Probabilità} \simeq 10^{-12} \,\%
    \end{split}
\end{equation}
Se diminuiamo la larghezza della barriera a $L = \qty{0.1}{\nano\meter}$, la probabilità di trasmissione diventa:
\begin{equation}
    T \simeq \efunction^{-3.25} \simeq 3.9 \cdot 10^{-2} \rightarrow \text{Probabilità} \simeq 4 \%
\end{equation}
Il modello a barriera rettangolare ci serve per ottenere la legge di Geiger-Nuttall, che mette in relazione il tempo di dimezzamento $T_{1/2}$ di un nuclide con l'energia cinetica $K_\alpha$ della particella $\alpha$ emessa.

\begin{theorem}{Legge di Geiger-Nuttall}
    Il tempo di dimezzamento $T_{1/2}$ di un nuclide che decade emettendo una particella $\alpha$ è inversamente proporzionale alla probabilità di trasmissione della particella attraverso la barriera di potenziale Coulombiano del nucleo, che dipende dall'energia cinetica $K_\alpha$ della particella stessa e dal numero atomico $Z$ del nucleo padre:
    \begin{equation}
        \log_{10} T_{1/2} \propto \dfrac{Z-2}{\sqrt{K_\alpha}}
    \end{equation}
\end{theorem}

Inizialmente questa legge fu ottenuta empiricamente, ma può essere giustificata tramite il modello di tunneling quantistico. Una particella $\alpha$ che attraversa la barriera di potenziale Coulombiano dal punto $A$ al punto $B$ può essere descritta come la somma di passaggi per barriere di potenziale rettangolari di larghezza infinitesima $\Delta r_i$. La probabilità di trasmissione attraverso la barriera complessiva è data dal prodotto delle probabilità di trasmissione attraverso ogni barriera infinitesimale:
\begin{equation}
...
\begin{equation}
    G = \int_R^{R_{\text{c}}} \gamma(r) \dd{r} = \frac{1}{\hbar} \int_R^{R_{\text{c}}} \sqrt{2M_{\alpha}(U_{\text{c}}(r) - K_\alpha)} \dd{r}
\end{equation}
Dove $R$ è il raggio del nucleo, $R_{\text{c}}$ è il punto in cui la barriera di potenziale Coulombiano incontra l'energia cinetica della particella $\alpha$, $M_\alpha$ è la massa della particella $\alpha$, e $U_{\text{c}}(r)$ è l'energia potenziale Coulombiana. Poiché:
\begin{equation}
    \dfrac{U_{\text{c}}(r)}{K_\alpha} = \dfrac{R_{\text{c}}}{r}
\end{equation}
Riscriviamo il fattore di Gamow come:
\begin{equation}
    G = \frac{\sqrt{2M_\alpha K_\alpha}}{\hbar} \int_R^{R_{\text{c}}} \sqrt{\dfrac{R_{\text{c}}}{r} - 1} \dd{r}
\end{equation}
Effettuando il cambio di variabile $x = r/R_{\text{c}}$, otteniamo:
\begin{equation}
    G = \frac{R_{\text{c}}}{\hbar}\sqrt{2M_\alpha K_\alpha}  \int_{R/R_{\text{c}}}^{1} \sqrt{\dfrac{1}{x} - 1} \dd{x}
\end{equation}
Inoltre $R \ll R_{\text{c}}$ quindi possiamo approssimare l'integrale come:
\begin{equation}
    \int_{0}^{1} \sqrt{\dfrac{1}{x} - 1} \dd{x} = \frac{\pi}{2}
\end{equation}
Quindi il fattore di Gamow diventa:
\begin{equation}
    G \simeq \frac{\pi R_{\text{c}}}{2\hbar} \sqrt{2M_\alpha K_\alpha}
\end{equation}
Ricordando che:
\begin{equation}
    R_{\text{c}} = \dfrac{1}{4\pi \varepsilon_0} \dfrac{2(Z-2) e^2}{K_\alpha}, \qquad \alpha_{\text{c}} = \dfrac{e^2}{4\pi \varepsilon_0 \hbar c} \simeq \dfrac{1}{137}, \qquad K_\alpha = \dfrac{1}{2}M_\alpha v^2
\end{equation}
Troviamo che:
\begin{equation}
    G \simeq \dfrac{\pi (Z-2) e^2}{4\pi \varepsilon_0 \hbar} \sqrt{\dfrac{2M_\alpha}{K_\alpha}} = \dfrac{(Z-2) \alpha_{\text{c}} \pi}{v/c}
\end{equation}
Infine, troviamo la legge di Geiger-Nuttall:
\begin{equation}
    T = \efunction^{-2G} \propto \lambda \propto \dfrac{1}{T_{1/2}} \implies \boxed{\log_{10} T_{1/2} \propto \dfrac{Z-2}{\sqrt{K_\alpha}}}
\end{equation}
...
\begin{equation}
    n \to p + e^- + \anti{\nu}_{e}
\end{equation}
Oppure realizzare rilevatori, costituiti da una vasca di 200 litri di acqua mescolata a cloruro di sodio e cadmio, circondata da rivelatori di scintillazione. Gli antineutrini interagiscono con i protoni nell'acqua, producendo positroni che generano segnali nei rivelatori:
\begin{equation}
    \anti{\nu}_{e} + p \to n + e^+
\end{equation}
...
Possiamo inoltre osservare che è possibile la reazione inversa del decadimento $\beta^+$, chiamata \textbf{cattura elettronica}:
\begin{equation}
    e^- + \atom{Y}{A}{Z} \to \atom{X}{A}{Z-1} + \nu_{e}
\end{equation}
...
\begin{equation}
    \begin{split}
        ...
        &\simeq  \sqbr{M_{\text{at}}(\atom{Y}{A}{Z}) - Zm_e}c^2 - \sqbr{M_{\text{at}}(\atom{X}{A}{Z+1}) - (Z+1)m_e}c^2 - m_e c^2 =\\[8pt]
...
Ma cosa implicava questa scoperta? Di fatto la scoperta di Wu dimostrava che le interazioni deboli hanno una preferenza verso una certa configurazione di spin e momento, violando la simmetria di parità. In particolare si osserva che gli \textbf{elettroni} hanno preferibilmente \textbf{elicità negativa} (sinistra) e i \textbf{neutrini} hanno preferibilmente \textbf{elicità positiva} (destra). Successivamente si scoprirà che i neutrini hanno sempre elicità positiva, mentre gli antineutrini hanno sempre elicità negativa.
...
\begin{table}[H]
    ...
    \begin{tabular}{cccccccc}
        \hline
        & \textbf{M} ($\si{\mega\electronvolt/c^2}$) & \textbf{Q} & $\mathbf{L_{\boldsymbol{e}}}$ & $\mathbf{L_{\boldsymbol{\mu}}}$ & $\mathbf{L_{\boldsymbol{\tau}}}$ & $\boldsymbol{\tau}$ (s) & \textbf{Decadimenti principali} \\
        \hline
        $e^-$ & 0.511 & $-1$ & $1$ & $0$ & $0$ & Stabile & - \\[8pt]
        $\mu^-$ & 105.7 & $-1$ & $0$ & $1$ & $0$ & $\qty{2.197e-6}{}$ & $e^-,\anti{\nu}_{e},\nu_{\mu}\,(100\%)$ \\[8pt]
        $\tau^-$ & 1777.0 & $-1$ & $0$ & $0$ & $1$ & $\qty{2.906e-13}{}$ & $\mu^-,\anti{\nu}_{\mu},\nu_{\tau}\,(17.4\%)$ \\[8pt]
        &&&&&&& $e^-,\anti{\nu}_{e},\nu_{\tau}\,(17.8\%)$ \\[8pt]
        &&&&&&& $\nu_\tau + \text{adroni}\,(\simeq 64\%)$ \\[8pt]
        $\nu_{e}$ & $< 2.2 \cdot 10^{-6}$ & $0$ & $1$ & $0$ & $0$ & Stabile & - \\[8pt]
        $\nu_{\mu}$ & $< 0.19$ & $0$ & $0$ & $1$ & $0$ & Stabile & - \\[8pt]
        $\nu_{\tau}$ & $< 18.2$ & $0$ & $0$ & $0$ & $1$ & Stabile & - \\
        \hline
    \end{tabular}
...
\begin{equation}
    \dfrac{N(\atom{C}{14}{})}{N(\atom{C}{12}{})} \simeq 1.3 \cdot 10^{-12}
\end{equation}
