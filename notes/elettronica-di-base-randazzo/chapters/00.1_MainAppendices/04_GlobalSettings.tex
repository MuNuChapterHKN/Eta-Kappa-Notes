...
% ________________________________________________ IMPOSTAZIONI GLOBALI ________________________________________________

% -------------------------------------------------- LAYOUT TESTATINE --------------------------------------------------
% Vedi Custom Class per documentazione.
\setupHeaders{
    HeaderCase = normal,
    ChapterLabel = brief,
    NumberLabel = normal,
...
% SOVRASCRITTURA IMPOSTAZIONI GLOBALI
\RefMattDefaultLinksColor   % NOTA: ho usato questo approccio per scrivere i colori che cambiano una volta sola.
                            % Vedi 02_CustomMacros.tex per la documentazione a riguardo.
\hypersetup{%
    colorlinks=true,        % Attiva il colore del testo per i link; rimuove i riquadri colorati (antiestetici nel PDF).
    citecolor=blue,         % Colore per i riferimenti bibliografici generati con il comando \cite e varianti.
...
% ---------------------------------------------------- SILLABAZIONE ----------------------------------------------------
% Per modificare la sillabazione di una parola in tutto il documento, basta usare il comando \hyphenation{}, anche
% inserenedo più parole modificate da spazi, utilizzando trattini per indicare la sillabazione voluta.
% UTILIZZO PER IMPEDIRE LA SILLABAZIONE DI UNA PAROLA: basta inserire la parola SENZA TRATTINI.

% Blocco sillabazione di determinati termini, es: Acronimi
\hyphenation{WYSIWYG WYSIWYM}
