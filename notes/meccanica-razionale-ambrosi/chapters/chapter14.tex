...
\end{example}
Consideriamo un sistema conservativo ad \(1\) grado di libertà. Possiamo scrivere
\[
T=\frac{1}{2}a(q)\dot q^2,
\qquad
V=V(q),
\]
con \(a(q)>0\) nella regione di configurazioni considerata.

Sia
\[
(q,\dot q)=(q_0,0)
\]
una configurazione di equilibrio stabile. Per studiare le piccole oscillazioni armoniche supponiamo inoltre che l'equilibrio sia \emph{non degenere}, cioè
\[
V''(q_0)>0.
\]
...
Sviluppiamo \(V(q)\) in serie di Taylor:
\[
V(q)
=
V(q_0)
...
V'(q_0)(q-q_0)
+
\frac{1}{2}V''(q_0)(q-q_0)^2
+
o\!\left((q-q_0)^2\right).
\]

Poiché il potenziale è definito a meno di una costante additiva, possiamo porre
\[
V(q_0)=0.
\]
Inoltre, essendo \(q_0\) una configurazione di equilibrio,
\[
V'(q_0)=0.
\]
...
Allo stesso modo, sviluppiamo l'energia cinetica
\[
2T(q,\dot q)=a(q)\dot q^2
\]
attorno a \((q_0,0)\). Ponendo
\[
x=q-q_0,
\]
si ha
...
\[
\frac{\partial \widetilde{\mathcal L}}{\partial \dot q}
=
a(q_0)\dot q,
\]
e quindi
...
mentre
\[
\frac{\partial \widetilde{\mathcal L}}{\partial q}
=
-
V''(q_0)x.
\]
...
Quindi, per un equilibrio stabile non degenere, il sistema linearizzato è un oscillatore armonico:
\[
\ddot x+\omega^2x=0.
\]

Linearizziamo ora le equazioni del moto per un sistema conservativo con \(m\) gradi di libertà. Siano
\[
\underline q=(q_1,\ldots,q_m)
\]
le coordinate libere.
...
\dot{\underline q},
\]
con \(A(\underline q)\) simmetrica definita positiva.

Sia
\[
(\underline q,\dot{\underline q})
=
(\underline q_0,\underline 0)
\]
una configurazione di equilibrio stabile. Per ottenere la linearizzazione armonica supponiamo inoltre che l'equilibrio sia \emph{non degenere}, cioè
\[
\nabla V(\underline q_0)=\underline 0,
\qquad
H_V(\underline q_0)\text{ definita positiva},
\]
dove \(H_V(\underline q_0)\) è la matrice Hessiana del potenziale.

Sviluppiamo \(V(\underline q)\) in serie di Taylor:
\[
V(\underline q)
=
V(\underline q_0)
+
\nabla V(\underline q_0)\cdot
(\underline q-\underline q_0)
+
\frac{1}{2}
(\underline q-\underline q_0)^T
H_V(\underline q_0)
(\underline q-\underline q_0)
+
o\!\left(
\|\underline q-\underline q_0\|^2
\right).
\]
...
V(\underline q_0)=0.
\]
Essendo \(\underline q_0\) una configurazione di equilibrio,
\[
\nabla V(\underline q_0)=\underline 0.
\]
...
Con ragionamento analogo al caso \(m=1\), per la parte cinetica otteniamo
\[
T
\simeq
\frac{1}{2}
...
Pertanto, ponendo
\[
\underline x=\underline q-\underline q_0,
\]
la lagrangiana linearizzata assume la forma
\[
\widetilde{\mathcal L}
=
\frac{1}{2}
...
Le equazioni di Lagrange del sistema linearizzato sono quindi
\[
A(\underline q_0)\ddot{\underline x}
=
-
H_V(\underline q_0)\underline x.
\]
...
H_V(\underline q_0)
\]
sono definite positive, il sistema linearizzato descrive un sistema di oscillatori armonici accoppiati, che può essere disaccoppiato tramite un opportuno cambiamento lineare alle coordinate normali.
...
\noindent (i) è simmetrica:
\begin{align*}
\left(A^{-1/2}ABA^{1/2}\right)^T &= \left(A^{1/2}\right)^T \cdot \left(A^{-1/2}AB\right)^T \\
...
\noindent (ii) è definita positiva:
\begin{align*}
\underline{x}^T A^{-1/2}ABA^{1/2}\underline{x} &= \underline{x}^T A^{1/2}BA^{1/2}\underline{x} \\
&= \underline{x}^T\left(A^{1/2}\right)^TBA^{1/2}\underline{x} \\
&= \left(A^{1/2}\underline{x}\right)^TBA^{1/2}\underline{x} \\
&= \underline{y}^TB\underline{y} \ge 0 \quad \forall \underline{y}\in \mathbb{R}^n
\end{align*}
con \(\underline{y}=A^{1/2}\underline{x}\).

Ci ricordiamo che due matrici simili hanno gli stessi autovalori e notiamo che
\[
\left(A^{1/2}\right)^{-1}ABA^{1/2}\sim AB.
\]
Concludiamo che \(AB\) è diagonalizzabile e ha tutti gli autovalori positivi.
\end{proof}
...
\begin{tikzpicture}[scale=1.2, >=stealth]
  % Linea del piano orizzontale
  \draw[thick] (-2.4,0) -- (2.4,0);

  % Circonferenze dei tre dischi
  \draw[thick] (-0.8,0.8) circle (0.8);
  \draw[thick] (0.8,0.8) circle (0.8);
  \draw[thick] (0,2.185) circle (0.8);

  % Aggiungo i punti centrali per chiarezza
  \filldraw (-0.8,0.8) circle (1pt);
  \filldraw (0.8,0.8) circle (1pt);
  \filldraw (0,2.185) circle (1pt);

  % Etichette massa e raggio (leggermente spostate all'esterno)
  \node at (-1.9,1.1) {\(m,r\)};
  \node at (1.9,1.1) {\(m,r\)};
  \node at (0.9,2.8) {\(m,r\)};

  % ---------------------------------------------------------
  % VETTORI VERTICALI (sfalsati per non sovrapporsi)
  % ---------------------------------------------------------
  % Disco in alto: sposto il testo mg MOLTO PIÙ IN BASSO (pos=0.9) per superare i vettori \xi
  \draw[red, thick, ->] (0,2.185) -- (0,1.1) node[pos=0.9, right, fill=white, inner sep=1pt] {\(mg\)};

  % Disco in basso a sinistra: inverto le posizioni! \varphi all'esterno e mg all'interno
  \draw[red, thick, ->] (-0.75,0.8) -- (-0.75,-0.5) node[right] {\(mg\)};
  \draw[orange, thick, ->] (-0.85,0) -- (-0.85,0.7) node[left, fill=white, inner sep=1pt] {\(\varphi\)};

  % Disco in basso a destra: rimangono perfetti così
  \draw[red, thick, ->] (0.85,0.8) -- (0.85,-0.5) node[right] {\(mg\)};
  \draw[orange, thick, ->] (0.75,0) -- (0.75,0.7) node[left, fill=white, inner sep=1pt] {\(\varphi\)};

  % ---------------------------------------------------------
  % VETTORI OBLIQUI E ORIZZONTALI (aggiunto fill=white a tutti)
  % ---------------------------------------------------------
  % Forze interne \xi
  \draw[orange, thick, ->] (-0.4,1.45) -- (0,2.15) node[midway, above left, fill=white, inner sep=1pt] {\(\xi\)};
  \draw[orange, thick, ->] (0.4,1.45) -- (0,2.15) node[midway, above right, fill=white, inner sep=1pt] {\(\xi\)};

  % Forza \psi (arancione) - spostata a y = 0.95 (sopra il centro)
  \draw[orange, thick, ->] (-0.1, 0.95) -- (-0.7, 0.95) node[midway, above, fill=white, inner sep=1pt] {\(\psi\)};

  % Forza della molla 2Kr (rossa) - spostata a y = 0.65 (sotto il centro)
  \draw[red, thick, ->] (-0.7, 0.65) -- (0.1, 0.65) node[midway, below, fill=white, inner sep=1pt] {\(2Kr\)};
