...
0
\end{pmatrix}
+\hat{\imath}\bigl(-\omega_z(y_p-y_q)\bigr)
+\hat{\jmath}\bigl(-(-\omega_z(x_p-x_q))\bigr)
+\hat{k}\cdot 0
\]
\[
=
...
+
\begin{pmatrix}
y_q\omega_z-\omega_z y_p\\
x_q\omega_z-\omega_z x_p\\
0
\end{pmatrix}
=
\begin{pmatrix}
v_{q,x}+y_q\omega_z-\omega_z y_p\\
v_{q,y}+x_q\omega_z-\omega_z x_p\\
0
\end{pmatrix}.
\]
...
\end{theorem}
\begin{proof}
Per assurdo suppongo che per $q_1,q_2$ si ha $\underline{\omega}_1\neq \underline{\omega}_2$.
...
\underline{0}
=
+\underline{\omega}_1(\underline{x}_p-\underline{x}_{q_2})
-\underline{\omega}_2(\underline{x}_p-\underline{x}_{q_2})
\]
...
\qquad \forall\,\underline{x}_p\ \forall\,\underline{x}_{q_2}
\]
...
% centri
\coordinate (O) at (0,0);
\coordinate (A) at ({2*sin(\theta)},{-2*cos(\theta)}); % distanza 2R
\coordinate (C) at ({2*sin(\theta)+sin(\theta)},{-2*cos(\theta)-cos(\theta)});

% circonferenze
\draw[line width=1.2pt] (O) circle (\Rbig);
\draw[line width=1.2pt] (A) circle (\Rsmall);
...
\draw[dashed] (O) -- (0,-2.45);

% etichette
\node[above] at (O) {$O$};
\node[right] at ($(A)+(0.18,0.05)$) {$A$};
\node[right] at ($(C)+(0.15,-0.05)$) {$C$};

% etichetta R
\node[right] at ($(A)!0.58!(C)+(0.15,0.1)$) {$R$};

% versore normale
\draw[->,line width=0.9pt] (A) -- ++({0.78*cos(\theta)},{0.78*sin(\theta)});
\node[left] at (1.5,-1) {$\hat{n}$};


% freccia omega sopra il disco piccolo
\draw[->,line width=0.9pt]
($(A)+(0.25,1.0)$) arc[start angle=160,end angle=20,radius=0.75];
\node[right] at ($(A)+(0.85,1.15)$) {$\omega$};

% freccia Omega a sinistra
\draw[->,line width=0.9pt]
($(O)+(-1.1,-0.1)$) arc[start angle=170,end angle=285,radius=0.75];
\node[left] at ($(O)+(-1.25,-0.95)$) {$\Omega$};

% angolo theta
\draw[->,line width=0.9pt]
(0,-0.7) arc[start angle=-90,end angle={-90+\theta},radius=0.7];
\node at (0.3,-0.8) {$\Theta$};
